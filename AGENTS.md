...
映见的品牌 Slogan 是“让变美更容易”。首页以用户任务组织，只提供两个大入口，并在选图前确定本次任务：

- **图片应用**：图片应用 → 选图片 → 定风格 → 应用风格 → 静态结果。应用优先本地完成，并保持可预览、可撤销、可恢复和可复现。
- **动起来**：动起来 → 选图片 → 定动态方向与风格 → 确认生成 → 动态结果。生成由用户明确发起，并明确等待、隐私和权益成本。

两个入口是相互独立的任务路径：定风格页面只承接当前任务，后续页面只突出该任务当前唯一主操作；切换任务必须返回首页。图片和结果始终是视觉主体，默认界面保持简单、安静、低决策负担；风格候选用于快速切换，更多解释和高级能力按需渐进展开。产品不得把传统工具分类、控件矩阵或内部参数暴露为主路径。

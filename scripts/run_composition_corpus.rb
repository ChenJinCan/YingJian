...
def fail_contract(message)
  warn "Composition corpus failed: #{message}"
  exit 1
end
...
if manifest_argument.nil? || output_argument.nil? || !ARGV.empty?
  fail_contract("usage: run_composition_corpus.rb MANIFEST OUTPUT_DIRECTORY")
end
...
begin
  assets = CompositionCorpus.select_assets(manifest.fetch("assets"))
rescue CompositionCorpus::ContractError => error
...
measure_source = File.join(repo_root, "scripts/support/measure_composition_candidate.swift")
contract_source = File.join(repo_root, "scripts/support/composition_corpus.rb")
FileUtils.mkdir_p(quality_root)

reports = []
Dir.mktmpdir(".composition-", quality_root) do |temporary_root|
  executable = File.join(temporary_root, "measure-composition")
  ...
  assets.each do |asset|
    ...
    begin
      ...
      CompositionCorpus.validate_result!(asset_id: asset_id, result: result)
...
puts "Composition engineering corpus passed: #{reports.length} assets"

...
def fail_contract(message)
  warn "Basic editing corpus failed: #{message}"
  exit 1
end
...
if manifest_argument.nil? || output_argument.nil? || !ARGV.empty?
  fail_contract("usage: run_basic_editing_corpus.rb MANIFEST OUTPUT_DIRECTORY")
end
...
begin
  assets = BasicEditingCorpus.select_assets(manifest.fetch("assets"))
rescue BasicEditingCorpus::ContractError => error
...
measure_source = File.join(repo_root, "scripts/support/measure_basic_editing_candidate.swift")
contract_source = File.join(repo_root, "scripts/support/basic_editing_corpus.rb")
FileUtils.mkdir_p(quality_root)

reports = []
Dir.mktmpdir(".basic-editing-", quality_root) do |temporary_root|
  executable = File.join(temporary_root, "measure-basic-editing")
  ...
  assets.each do |asset|
    ...
    begin
      ...
      BasicEditingCorpus.validate_result!(asset_id: asset_id, result: result)
...
puts "Basic editing engineering corpus passed: #{reports.length} assets"

...
def fail_contract(message)
  warn "Semantic editing corpus failed: #{message}"
  exit 1
end
...
if manifest_argument.nil? || output_argument.nil? || !ARGV.empty?
  fail_contract("usage: run_semantic_editing_corpus.rb MANIFEST OUTPUT_DIRECTORY")
end
...
begin
  assets = SemanticEditingCorpus.select_assets(manifest.fetch("assets"))
rescue SemanticEditingCorpus::ContractError => error
...
measure_source = File.join(repo_root, "scripts/support/measure_semantic_editing_candidate.swift")
contract_source = File.join(repo_root, "scripts/support/semantic_editing_corpus.rb")
FileUtils.mkdir_p(quality_root)

background_assets = assets.select { |asset| (asset.fetch("tags") & %w[landscape food pet]).any? }
fail_contract("semantic corpus needs at least two replacement backgrounds") if background_assets.length < 2

reports = []
Dir.mktmpdir(".semantic-editing-", quality_root) do |temporary_root|
  executable = File.join(temporary_root, "measure-semantic-editing")
  ...
  assets.each do |asset|
    ...
    background = background_assets.find { |candidate| candidate.fetch("id") != asset_id } || background_assets.first
    background_path = File.realpath(File.expand_path(background.fetch("file"), corpus_root))
    output, error, measure_status = Open3.capture3(executable, source_path, background_path)
    fail_contract("#{asset_id} measurement failed: #{error.lines.first&.strip}") unless measure_status.success?
    begin
      result = JSON.parse(output)
      SemanticEditingCorpus.validate_result!(asset_id: asset_id, result: result)
    ...
    reports << {
      ...
      "background_id" => background.fetch("id"),
      "background_sha256" => Digest::SHA256.file(background_path).hexdigest,
      "result" => result,
    }
  end
  begin
...
puts "Semantic editing engineering corpus passed: #{reports.length} assets"

...
映见不是把专业修图软件的全部工具压缩到手机上，而是帮助用户完成一项清晰任务：

> 从一张或一组照片出发，快速选择可靠方向，精修重点照片，统一整组，并放心导出。
...
1. 首页不使用常驻底部导航栏，只突出“开始修图”和最近项目。
2. 编辑器以照片为视觉中心，工具和操作采用上下文底部区域，不建立第二套页面导航。
3. 用户先看到三套结果，再按需展开参数；不会先面对几十个工具。
4. 语音、文字和手动参数是同一编辑配方的并列入口。
5. AI 每次修改都必须表现为用户看得见、能继续调整的参数变化。
6. “整组”与“当前照片”是持续可见的编辑范围，不使用隐藏默认值。
7. 所有修改都能撤销、重做、单项归零和整体恢复。
...
原因：用户只有一个高频主任务“修图”，首页、项目和设置不足以形成三个同权目的地。增加 Tab Bar 会把任务流误导成内容型 App，并长期占用照片预览空间。
...
2. **任务层**：推荐方向、构图、光色、滤镜与 HSL、画质、人像、主体与局部。
3. **操作层**：撤销、重做、重置、语音/文字、导出。

任意时刻只允许一个主要工具面板处于展开状态。
...
工具横向滚动时，当前选中项保持可见；工具状态通过文字、图标和选中样式共同表达。

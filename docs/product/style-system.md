...
> 状态：产品与领域合同。本文定义风格来源、`StyleDefinition`、不可见执行语言，以及四个用户任务和两个执行分支的边界。首阶段页面、状态和验收见 [MVP Spec](mvp-spec.md)。

## 1. 核心模型

风格是换风格与做动态效果任务的用户创作单位。优化照片和去背景 / 去杂物使用任务专属的本地操作，不把这些操作伪装成风格。
...
用户界面只围绕 `StyleBrief`、`StyleDefinition`（如适用）和结果沟通。`StyleRecipe`、参数范围、能力目录、模型和供应商只属于执行系统。`CreationTask=optimize|style|cleanup|motion` 保存用户任务；`CreationIntent=apply|motion` 只选择静态或生成执行边界。
...
首页先确定 `CreationTask`，并从它派生 `creationIntent=apply|motion`。只有换风格或做动态效果任务需要风格；满足以下条件后，风格才进入与任务对应的 `applyStyleReady` 或 `motionStyleReady`：
...
首页以四个任务先确定用户要得到的结果；任务选择发生在选图前，进入后只显示当前任务的主操作，不共享 ready 页面。

| `CreationTask` | 用户可见任务 | `CreationIntent` | 当前合同 |
|---|---|---|---|
| `optimize` | 优化照片 | `apply` | 本地质量优化：调亮、清晰、增强质感；可撤销、不可上传 |
| `style` | 换风格 | `apply` | 用 `StyleDefinition` 形成可复现静态结果 |
| `cleanup` | 去背景 / 去杂物 | `apply` | 语义白底/背景处理和局部本地擦除；不提供透明 alpha 抠图或无限制生成式去物 |
| `motion` | 做动态效果 | `motion` | 独立生成语义；当前服务未接入，必须显示不可用且不上传、不创建任务、不扣费 |

优化、换风格和清理共享静态执行安全边界，但不因此变成一个笼统的“图片应用”入口。做动态效果路径直接使用其源图和当前风格定义，静态结果不是生成任务的前置输入。旧 `creationIntent=apply` 草稿迁移为 `CreationTask.style`，保存原有风格含义。

硬边界：

- 三个静态任务不得静默创建生成任务或上传原图。
- 清理任务只执行已验证的本地语义背景或擦除能力；不支持时 fail closed，不虚构完整抠图或生成式补全。
- 做动态效果在服务未接入时不得显示可执行生成 CTA、费用、进度或产物；服务接入后，生成不得覆盖原图、当前 `StyleDefinition` 或任何已有结果。
- 静态预览不得扣除动态生成额度。
- 四个任务的成功、失败、保存和分享状态分别记录；切换任务必须返回首页。

## 7. 动态生成服务接入后的合同

当前动态服务未接入；本节是服务接入后才可启用的“让图片动起来”闭环合同。动态任务的输入为：
...
- 动态服务接入后的任务和结果引用确切源图、风格 revision、生成策略版本和任务 ID；服务未接入时不能写入虚构任务身份。
...
- 优化、换风格和清理路径永远不创建网络生成任务；动态服务未接入时也不创建网络图片任务。
- 服务接入后的生成取消、失败和恢复不会修改源图、当前风格或任何已有结果，也不会重复扣费。
- 项目冷启动能恢复确切 `CreationTask`、风格快照和原生成任务身份（如有）。
...
3. 优化、换风格和清理从原图得到可保存的受支持静态结果，且不暴露底层参数；
4. 四个任务从首页入口开始就在 UI、状态、持久化、隐私、费用、失败和结果身份上分离；旧 `apply` 草稿安全迁移为换风格；
5. 动态服务未接入时，做动态效果路径证明真实不可用且没有上传、任务或成本；服务接入后，再证明当前风格能创建、取消、恢复并完成一个动态任务；

...
- On iOS, every non-neutral adjustment requires a parameter-specific direction, visible-strength and clipping contract over the frozen corpus plus blinded review; `ios-contrast-semantic-v1` is the first frozen contract, and neutral pixel tolerance is not evidence of retouch quality.

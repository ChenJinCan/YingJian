...
开始一轮设备测试前，冻结同一 `source_commit`。iOS 三档设备复用同一个已打包 Profile/Release Apple 产物，Android 三档设备复用另一个已打包 Profile/Release Android 产物；manifest 中的产物 SHA-256 必须与本地 `build_artifact` 文件一致。设备 run 不能混入其他 commit、版本、build 或构建模式。
...
完整门要求恰好六个物理设备 run：iOS 与 Android 各 low/mid/high 一项。每台设备使用本地随机标签的 SHA-256 作为 `device_id`，用于发现重复设备但不提交 UDID/序列号；六项必须互不相同。替代机型必须在 `tier_basis` 写清为什么属于冻结档位；`device.physical` 必须为 `true`。
...
`methodology` 必须记录实际计时、内存、帧率、温控工具和可重复生命周期步骤。使用 Profile/Release 的 Instruments/xctrace、Flutter frame timing、Android Perfetto/系统内存采样或等价原始来源；不要用 Debug DevTools 数字替代。
...
检查器按 nearest-rank 从原始序列计算 p50/p95，按低/中/高冻结预算检查首预览、六张推荐、滑块、最低帧率、12/48 MP、六张批量、100 ms UI stall、设备内存 25%/512 MB 双上限和温控。完整门只接受 6/6；缺 run、重复档位、跨 build、证据哈希漂移、严重温控无降级或任一恢复/隐私结果为 false 都会失败。

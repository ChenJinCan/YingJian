...
> 日期：2026-08-06
> 范围：Tickets 17–18，仅用于冻结进入产品纵切前的端侧候选边界。这里的工程结论不是六项能力已经通过同图竞品盲评，也不是真机性能结论。
...
- 旧自建综合滤镜出现脸/头发边界发黑，已淘汰。生产质感磨皮现为受保护 mask 内的 `texture-smoothing-v2`，并在进入惰性 Core Image 链前物化输入以避免多人图空间错位。OpenCV 双边滤波仍只作为边缘保持参照，未成为生产依赖；实际强度仍由 Ticket 24 固定样片门决定。

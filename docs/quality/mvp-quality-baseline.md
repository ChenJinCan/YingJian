...
| 当前预览 | Flutter 色彩矩阵，不是原生持有的 GPU 纹理 | 不满足目标架构 |
| iOS 导出 | Core Image 整图渲染后 JPEG 100 写入 PhotoKit | 仅开发演示 |
| Android 导出 | Bitmap 整图解码、方向变换、第二张输出 Bitmap、JPEG 100 | 48 MP 内存门未通过 |
| 当前物理设备 | 未发现可用 iPhone 或 Android；仅 iOS 模拟器 | 物理验证阻断 |
